...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('chat')
        .setDescription('Chat with an AI assistant powered by Claude')
    ...
    async execute(interaction) {
        ...
        if (!isAvailable()) {
            return interaction.reply({
                content: 'AI chat is not available. The bot administrator needs to configure the API key.',
        ...
        if (estimatedTokens > aiConfig.tokens.maxInput) {
            return interaction.reply({
                content: `Your message is too long (estimated ${estimatedTokens} tokens, max ${aiConfig.tokens.maxInput}). Please shorten it.`,
                flags: MessageFlags.Ephemeral
            });
        }
