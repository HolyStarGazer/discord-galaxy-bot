...
function setupPlayerEvents(ctx) {
    ...
    player.on(AudioPlayerStatus.Idle, async () => {
        ...
        if (nextTrack) {
            ...
            if (ctx.channel.isTextBased()) {
                ...
                logWithTimestamp('INFO', `Cannot send messages to this channel type.`);
            }
        }
    });
    ...
    player.on('error', (error) => {
        logWithTimestamp('ERROR', `Audio player error`, 2, error);
...
async function playTrack(guildId, track) {
    ...
    try {
        ...
        const queue = queueManager.getQueue(guildId);
        if (resource.volume) {
            resource.volume.setVolume(queue.volume / 1000);
        }
        ...
        logWithTimestamp('ERROR', `Failed to play track: ${track.title}`, 2, error);
        throw error;
    }
}
 ...
 */
function createNowPlayingEmbed(track, queue, isPaused = false) {
    const { EmbedBuilder } = require('discord.js');

    const trackLink = track.shareUrl || track.license;

    const embed = new EmbedBuilder()
        .setColor(isPaused ? 0xFFAA00 : 0x00D4AA)  // Orange if paused, green if playing
        .setTitle(isPaused ? 'Paused' : 'Now Playing')
        .setDescription(`**[${track.title}](${trackLink})**\nby ${track.artist}`)
        .addFields(
            { name: 'Album', value: track.album || 'Unknown', inline: true },
            { name: 'Duration', value: track.durationFormatted || '--:--', inline: true },
            { name: 'In Queue', value: `${queue.remaining}`, inline: true }
        )
        .setFooter({ 
            text: `Jamendo (Royalty-Free) • CC License`,
